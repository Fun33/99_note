...
(function(){
    ...
    var patrn=/^http:\/\/[a-zA-Z]{2}\..+$/;  
    if (patrn.exec(sitebot_JsHost))
    {
    	//sitebot_JsHost = 'tracking.' + sitebot_JsHost;
	sitebot_JsHost = sitebot_JsHost.replace('http://', 'http://tracking.');
    }
    else
    {
	sitebot_JsHost = sitebot_JsHost.replace(/\btrack\b/g, 'tracking');
	sitebot_JsHost = sitebot_JsHost.replace(/\bwww\b/g, 'tracking');
    }

    if((sitebot_JsHost=='http://tracking.sitebro.com.tw/') || (sitebot_JsHost=='http://tracking.sitebro.tw/'))
    {
    	sitebot_JsHost='http://tracking.zh.sitebro.com/';
    }
